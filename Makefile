...
# Tarefas principais
.PHONY: all
all: docker k3d kubectl metallb kube-prometheus istio kiali argocd giropops-senhas giropops-locust chaos-mesh kubernetes-dashboard

OS := $(shell uname -s)

ifeq ($(OS),Linux)
  DOCKER_COMMAND = "sudo curl -fsSL https://get.docker.com | bash"s
  K3D_COMMAND = "wget -q -O - https://raw.githubusercontent.com/k3d-io/k3d/main/install.sh | bash"
...
.PHONY: k3d
k3d:
	@echo "Instalando o K3d..."
	@command -v k3d >/dev/null 2>&1 || $(K3D_COMMAND)
	@if [ -z "$$(k3d cluster list | grep k3dcluster)" ]; then k3d cluster create --config k3d-config/k3d_cluster.yaml ; fi
	@echo "K3d instalado com sucesso!"
...
.PHONY: clean
clean:
	@echo "Removendo o K3d..."
	@k3d cluster delete k3dcluster
	@echo "K3d removido com sucesso!"
ifeq ($(OS),Darwin)
